(function () {
    ...
    // ==================== 工具函数（使用 App 共享模块） ====================
    var safeGetJSON = App.safeGetJSON;
    var safeSetJSON = App.safeSetJSON;
    var getUsers = App.getUsers;
    var saveUsers = App.saveUsers;
    var getApplications = App.getApplications;
    var getProducts = App.getProducts;
    var saveProducts = App.saveProducts;
    var getDefaultCredit = App.getDefaultCredit;
    var getCreditLevel = App.getCreditLevel;
    var getActivities = App.getActivities;
    var addActivity = App.addActivity;
    ...
    function checkAdminLogin() {
        return App.checkAdminLogin(App.KEYS.ADMIN_LOGGED_IN);
    }

    function adminLogout() {
        App.adminLogout(App.KEYS.ADMIN_LOGGED_IN);
    }
    ...
    function switchModule(module) {
        App.switchModule(module, MODULES, MODULE_RENDER);
    }
    ...
    function renderMonitor() {
        ...
        Object.values(apps).forEach(function(userApps) {
            userApps.forEach(function(a) {
        ...
        var actEl = document.getElementById('activityList');
        var actEmptyEl = document.getElementById('activityEmpty');
        if (activities.length > 0) {
            actEl.innerHTML = activities.slice(0, 8).map(function(a) {
                return '<div class="px-6 py-3 flex items-center gap-3">' +
                    ...
                    '<span class="text-sm text-gray-700 flex-1">' + (a.desc || a.detail || '') + '</span>' +
                    '<span class="text-xs text-gray-400">' + App.formatDate(a.time) + '</span>' +
                '</div>';
            }).join('');
    ...
    function renderCredit(searchTerm) {
        var users = getUsers();
        var filtered = searchTerm ? users.filter(function(u) { return u.username.includes(searchTerm); }) : users;

        var listEl = document.getElementById('creditList');
        var emptyEl = document.getElementById('creditEmpty');

        if (filtered.length > 0) {
            listEl.innerHTML = filtered.map(function(u) {
                var score = getCreditData(u.username);
                var level = getCreditLevel(score);
                var color = App.getCreditColor ? App.getCreditColor(score) : (level.color || '#6b7280');
                return '<tr class="hover:bg-gray-50">' +
                    ...
                    '<td class="px-6 py-4"><div class="flex items-center gap-2">' +
                        ...
                        '<div class="w-16 h-2 bg-gray-200 rounded-full overflow-hidden">' +
                            '<div class="h-full rounded-full transition-all" style="width:' + Math.min(score / 10, 100) + '%;background:' + color + '"></div>' +
                        '</div></div></td>' +
                    '<td class="px-6 py-4"><span class="status-badge ' + level.class + '">' + level.text + '</span></td>' +
                    '<td class="px-6 py-4 text-xs text-gray-500">' + (u.creditUpdatedAt ? App.formatDate(u.creditUpdatedAt) : '-') + '</td>' +
                    '<td class="px-6 py-4"><button data-action="openCreditModal" data-username="' + u.username + '" class="px-3 py-1.5 text-xs font-medium text-purple-700 bg-purple-50 border border-purple-200 rounded-lg hover:bg-purple-100 transition-all">调整</button></td>' +
                '</tr>';
            }).join('');
    ...
    function handleCreditSubmit(e) {
        ...
        if (!reason) { App.showToast('请填写调整原因', 'error'); return; }
        ...
        document.getElementById('productList').innerHTML = products.map(function(p) {
            return '<div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">' +
                '<div class="flex items-start justify-between mb-4">' +
                    '<div class="w-12 h-12 rounded-xl bg-gradient-to-br ' + (p.color || 'from-purple-400 to-purple-600') + ' flex items-center justify-center shadow-md">' +
                ...
                '</div></div>';
        }).join('') || '<div class="col-span-full text-center py-12 text-sm text-gray-400">暂无产品</div>';
    }
    ...
    function handleProductSubmit(e) {
        ...
        if (!name) { App.showToast('请输入产品名称', 'error'); return; }
        ...
        if (id) {
            ...
            if (idx >= 0) {
                Object.assign(products[idx], { name: name, minAmount: minAmount, maxAmount: maxAmount, rate: rate, term: term, desc: desc, status: status });
            }
            addActivity('产品修改', '修改产品：' + name);
        } else {
            var newId = Math.max.apply(null, products.map(function(p) { return p.id; }).concat([0])) + 1;
            products.push({ id: newId, name: name, minAmount: minAmount, maxAmount: maxAmount, rate: rate, term: term, desc: desc, status: status, color: colors[products.length % colors.length] });
    ...
    function renderBankUsers() {
        var bankUsers = getBankUsers();
        var listEl = document.getElementById('bankUserList');
        var emptyEl = document.getElementById('bankUserEmpty');

        if (bankUsers.length > 0) {
            listEl.innerHTML = bankUsers.map(function(b) {
                return '<tr class="hover:bg-gray-50">' +
    ...
    function handleBankUserSubmit(e) {
        ...
        if (id) {
            ...
            if (idx >= 0) {
                Object.assign(bankUsers[idx], { username: username, password: password, role: role, limit: limit, status: status });
            }
            addActivity('用户修改', '修改银行用户：' + username);
        } else {
            var newId = Math.max.apply(null, bankUsers.map(function(b) { return b.id; }).concat([0])) + 1;
            bankUsers.push({ id: newId, username: username, password: password, role: role, limit: limit, status: status, createdAt: new Date().toISOString().substring(0, 10) });
    ...
    function renderAnalysis() {
        ...
        document.getElementById('creditDistribution').innerHTML = distItems.map(function(d) {
            var width = Math.round((d.count / maxDist) * 100);
            return '<div class="flex items-center gap-3">' +
                '<span class="text-xs text-gray-600 w-28">' + d.label + '</span>' +
                '<div class="progress-bar flex-1"><div class="progress-fill" style="width:' + width + '%;background:' + d.color + '"></div></div>' +
    ...
    function renderLogs() {
        ...
        var filtered = filterType === 'all' ? activities : activities.filter(function(a) { return a.type.includes(filterType); });

        var listEl = document.getElementById('logList');
        var emptyEl = document.getElementById('logEmpty');

        if (filtered.length > 0) {
            listEl.innerHTML = filtered.slice(0, 20).map(function(a) {
                return '<tr class="hover:bg-gray-50">' +
                    ...
                    '<td class="px-6 py-4 text-sm text-gray-600">' + (a.desc || a.detail || '') + '</td>' +
                    '<td class="px-6 py-4 text-xs text-gray-500">192.168.1.100</td>' +
                    '<td class="px-6 py-4 text-xs text-gray-400">' + App.formatDate(a.time) + '</td>' +
                '</tr>';
            }).join('');
            emptyEl.classList.add('hidden');
        } else {
    ...
    function exportLogs() {
        var activities = getActivities();
        if (activities.length === 0) {
            App.showToast('暂无日志数据', 'error');
            return;
        }
        var csv = '操作类型,操作内容,操作时间\n';
        activities.forEach(function(a) {
            csv += a.type + ',' + (a.desc || a.detail || '') + ',' + App.formatDate(a.time) + '\n';
        });
    ...
    function renderReports() {
        var records = getExportRecords();
        var listEl = document.getElementById('exportList');
        var emptyEl = document.getElementById('exportEmpty');
            ...
            listEl.innerHTML = records.map(function(r) {
                return '<div class="px-6 py-3 flex items-center justify-between">' +
                    ...
                    '<div class="flex items-center gap-4">' +
                        '<span class="text-xs text-gray-400">' + App.formatDate(r.time) + '</span>' +
    ...
    function exportReport(type) {
        ...
        if (type === 'users') {
            ...
            users.forEach(function(u) {
                csv += u.username + ',' + (u.realName || '') + ',' + (u.phone || '') + ',' + getCreditData(u.username) + ',' + App.formatDate(u.registeredAt || new Date()) + '\n';
            });
        } else if (type === 'loans') {
            csv = '申请人,产品,金额(万),期限,状态,申请时间\n';
            Object.entries(apps).forEach(function(entry) {
                var username = entry[0];
                var userApps = entry[1];
                userApps.forEach(function(a) {
                    csv += username + ',' + a.product + ',' + a.amount + ',' + a.term + '个月,' + a.status + ',' + App.formatDate(a.createdAt) + '\n';
                });
            });
        ...
        if (announcements.length > 0) {
            ...
            listEl.innerHTML = announcements.map(function(a, idx) {
                ...
                return '<tr class="hover:bg-gray-50">' +
                    ...
                    '<td class="px-6 py-4 text-xs text-gray-400">' + App.formatDate(a.createdAt) + '</td>' +
                    '<td class="px-6 py-4">' +
