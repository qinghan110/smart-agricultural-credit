...
var defaultProducts = [
    {
        ...
        img: 'https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=Lush green rice paddy field with golden sunlight, traditional Chinese countryside farmland, healthy crops growing, blue sky, professional agricultural photography, vibrant natural colors&image_size=landscape_4_3',
        fallback: 'https://images.unsplash.com/photo-1625246333195-78d9c38ad449?w=800&q=80'
    },
    {
        ...
        img: 'https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=Modern red tractor harvesting in golden wheat field, agricultural machinery at work, blue sky, productive farming scene, professional photography, cinematic lighting&image_size=landscape_4_3',
        fallback: 'https://images.unsplash.com/photo-1581094794329-c8112a89af12?w=800&q=80'
    },
    {
        ...
        img: 'https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=Beautiful green vegetable farm rows with farmers working, organic crops growing in fertile soil, sunny day, agricultural cultivation scene, professional photography&image_size=landscape_4_3',
    ...
    },
    {
        ...
        img: 'https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=Healthy dairy cows grazing on green pasture farm, livestock agriculture, blue sky with clouds, professional rural photography, vibrant natural colors&image_size=landscape_4_3',
    ...
    },
    {
        ...
        img: 'https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=Aerial view of modern agricultural farm complex with greenhouses and fields, large scale farming operation, professional agricultural photography, sunny day&image_size=landscape_4_3',
        fallback: 'https://images.unsplash.com/photo-1500382017468-9049fed747ef?w=800&q=80'
    },
    {
        ...
        img: 'https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=Farmer planting seeds in spring field, green sprouts growing in fertile soil, morning sunlight, agricultural cultivation, professional photography&image_size=landscape_4_3',
        fallback: 'https://images.unsplash.com/photo-1464226184884-fa280b87c3cc?w=800&q=80'
    },
    {
        ...
        img: 'https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=Modern food processing factory interior with stainless steel equipment, clean industrial facility, agricultural product manufacturing, professional photography&image_size=landscape_4_3',
    ...
    },
    {
        ...
        img: 'https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=Beautiful rural tourism village with traditional Chinese architecture, green mountains, farmland landscape, peaceful countryside scene, professional photography&image_size=landscape_4_3',
        fallback: 'https://images.unsplash.com/photo-1523747443260-7245b8e9b18a?w=800&q=80'
    },
    {
        ...
        img: 'https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=Lush green tea plantation terraces on mountain hillside, tea farmers picking tea leaves, misty morning, professional agricultural photography, scenic landscape&image_size=landscape_4_3',
        fallback: 'https://images.unsplash.com/photo-1571934811356-5cc061b6a7a8?w=800&q=80'
    }
];
...
function getAllProducts() {
    ...
    var existingNames = {};
    defaultProducts.forEach(function(p) { existingNames[p.name] = true; });
    try {
        var custom = JSON.parse(localStorage.getItem('agriLoanProducts')) || [];
        custom.forEach(function(p) {
            if ((p.status === 'active' || !p.status) && p.name && !existingNames[p.name]) {
                // 兼容 admin.html 保存的字段名
                var colorKey = 'fin';
                if (p.color) {
                    if (p.color.indexOf('agri') >= 0) colorKey = 'agri';
                    else if (p.color.indexOf('amber') >= 0) colorKey = 'amber';
                    else if (p.color.indexOf('rose') >= 0) colorKey = 'rose';
                    else if (p.color.indexOf('purple') >= 0) colorKey = 'fin';
                }
                var termMax = 36;
                if (p.term) {
                    var m = String(p.term).match(/(\d+)/);
                    if (m) termMax = parseInt(m[1]);
                }
                products.push({
                    ...
                    img: p.image || 'https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=Chinese agricultural farm landscape with green crops, professional photography, natural lighting&image_size=landscape_4_3',
                    fallback: 'https://images.unsplash.com/photo-1500382017468-9049fed747ef?w=800&q=80',
                    isCustom: true
                });
...
function renderProducts() {
    ...
    var filtered = currentProductFilter === 'all' ? products : products.filter(function(p) { return p.category === currentProductFilter; });
    ...
    grid.innerHTML = filtered.map(function(p) {
        var c = colorMap[p.color] || colorMap.agri;
        var customBadge = p.isCustom ? '<span class="absolute top-3 right-3 z-10 px-2 py-0.5 text-[10px] font-semibold rounded-full bg-purple-100 text-purple-700 border border-purple-200">新品</span>' : '';
        return '<div class="product-card group bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-[0_12px_40px_rgba(0,0,0,0.08)] hover:' + c.hoverBorder + ' transition-all duration-300 cursor-pointer flex flex-col">' +
            '<div class="relative h-44 overflow-hidden bg-gray-100">' +
                '<img class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" src="' + p.img + '" alt="' + p.name + '" onerror="this.src=\'' + p.fallback + '\'">' +
            ...
            '<div class="p-5 flex-1 flex flex-col">' +
                ...
                '</div>' +
                '<button class="w-full py-2.5 ' + c.btnBg + ' ' + c.btnText + ' font-semibold text-sm rounded-xl border ' + c.border + ' ' + c.btnHover + ' transition-all" onclick="selectProduct(\'' + p.name.replace(/'/g, "\\'") + '\')">' +
                    '立即申请' +
                '</button>' +
            '</div>' +
        '</div>';
    ...
    // 显示自定义产品提示
    var customCount = products.filter(function(p){ return p.isCustom; }).length;
    var tip = document.getElementById('customProductTip');
    if (customCount > 0) {
...
function fillProductSelects() {
    var products = getAllProducts();
    var applySelect = document.getElementById('applyProduct');
    var demandSelect = document.getElementById('demandType');
    // 保留第一个占位option
...
document.getElementById('demandForm').addEventListener('submit', function(e) {
    ...
    var type = document.getElementById('demandType').value,
        amount = document.getElementById('demandAmount').value.trim(),
        term = document.getElementById('demandTerm').value,
        purpose = document.getElementById('demandPurpose').value.trim();
    if (!type) { alert('请选择贷款类型'); return; }
    if (!amount || parseFloat(amount) <= 0) { alert('请输入有效的需求金额'); return; }
    if (!term) { alert('请选择期望期限'); return; }
    // 保存到 localStorage（银行中心可见）
    try {
        var demands = JSON.parse(localStorage.getItem('agriLoanDemands')) || [];
        demands.unshift({
            id: Date.now(),
            type: type,
            ...
            username: (getLoggedInUser() || {}).username || '游客',
            createdAt: new Date().toISOString(),
            status: 'pending'
        });
        localStorage.setItem('agriLoanDemands', JSON.stringify(demands));
    } catch(e) {}
    var r = document.getElementById('publishResult'), t = document.getElementById('publishResultText');
    t.textContent = '需求发布成功！您申请的'+amount+'万元'+type+'需求已提交，稍后将自动跳转至智能匹配。';
...
// ==================== 智能匹配 ====================
function doMatch() {
    var typeMap = {crop:'种植贷', machine:'农机贷', breed:'养殖贷', mixed:'惠农贷'},
        type = typeMap[document.getElementById('matchType').value] || '惠农贷',
        income = parseFloat(document.getElementById('matchIncome').value) || 10,
        credit = document.getElementById('matchCredit').value,
        rateMap = {A: '3.85%', B: '4.35%', C: '4.75%'},
        rate = rateMap[credit] || '4.35%',
        maxAmount = credit === 'A' ? Math.round(income * 2.5) : credit === 'B' ? Math.round(income * 1.5) : Math.round(income * 0.8),
        termMonths = credit === 'A' ? 36 : credit === 'B' ? 24 : 12;

    var results = [
        { product: type, amount: maxAmount, rate: rate, term: termMonths, score: credit === 'A' ? 95 : credit === 'B' ? 82 : 68, tag: credit === 'A' ? '强烈推荐' : credit === 'B' ? '推荐' : '可选' },
    ...
    results.forEach(function(r, i) {
        var colors = ['border-agri-400 bg-agri-50', 'border-fin-200 bg-fin-50/30', 'border-amber-200 bg-amber-50/30'];
        var tagColors = ['bg-agri-100 text-agri-700', 'bg-fin-100 text-fin-700', 'bg-amber-100 text-amber-700'];
        container.innerHTML += '<div class="match-card flex items-center justify-between p-5 rounded-2xl border '+colors[i]+'">'+
...
document.getElementById('applyForm').addEventListener('submit', function(e) {
    ...
    // 保存到 localStorage（银行中心可见，个人中心可见）
    var user = getLoggedInUser();
    var username = user ? user.username : name;
    try {
        var allApps = JSON.parse(localStorage.getItem('agriLoanApplications')) || {};
        var userApps = allApps[username] || [];
        var newApp = {
            id: Date.now(),
            applicant: name,
            idCard: idCard,
        ...
        localStorage.setItem('agriLoanApplications', JSON.stringify(allApps));
    } catch(err) {}

    var r = document.getElementById('applyResult'), t = document.getElementById('applyResultText');
    if (user) {
        t.textContent = '贷款申请已提交！您可在"个人中心"查看审批进度，客户经理将尽快联系您（'+phone+'）。';
    } else {
        t.textContent = '贷款申请已提交！客户经理将在24小时内与您联系（'+phone+'），请保持电话畅通。';
    }
    r.classList.remove('hidden'); r.classList.add('flex');
    setTimeout(function() { r.classList.add('hidden'); r.classList.remove('flex'); document.getElementById('applyForm').reset(); }, 4000);
});

// 手机号过滤
document.getElementById('applyPhone').addEventListener('input', function() { this.value = this.value.replace(/\D/g, '').slice(0, 11); });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    new Chart(trendCtx, {
        ...
        data: {
            ...
            datasets: [{
                label: '惠农贷', data: [120,135,155,140,170,185,195,210,225,240,255,270],
                borderColor: '#22c55e', backgroundColor: 'rgba(34,197,94,0.08)', fill: true, tension: 0.4, borderWidth: 2.5, pointRadius: 4, pointBackgroundColor: '#22c55e'
            }, {
                label: '农机贷', data: [80,90,105,95,115,125,130,140,150,160,175,190],
                borderColor: '#3b82f6', backgroundColor: 'rgba(59,130,246,0.06)', fill: true, tension: 0.4, borderWidth: 2.5, pointRadius: 4, pointBackgroundColor: '#3b82f6'
            }, {
                label: '种植贷', data: [60,70,80,90,100,110,120,130,145,155,165,180],
                borderColor: '#f59e0b', backgroundColor: 'rgba(245,158,11,0.06)', fill: true, tension: 0.4, borderWidth: 2.5, pointRadius: 4, pointBackgroundColor: '#f59e0b'
            }, {
                label: '养殖贷', data: [45,50,58,65,72,80,88,95,105,115,125,135],
                borderColor: '#f43f5e', backgroundColor: 'rgba(244,63,94,0.06)', fill: true, tension: 0.4, borderWidth: 2.5, pointRadius: 4, pointBackgroundColor: '#f43f5e'
            }]
        },
        options: {
            ...
            scales: {
                y: { beginAtZero: false, min: 30, grid: { color: '#f3f4f6' }, ticks: { callback: function(v) { return v + '万'; } } },
    ...
    new Chart(typeCtx, {
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: { callbacks: { label: function(ctx) { return ctx.label + ': ' + ctx.raw + '万元 (' + Math.round(ctx.raw/775*100) + '%)'; } } }
            },
            cutout: '55%'
        }
    });
